package multiQueryOptJoin.src.Common;
...
public class IntermediateResults {
	...
	public IntermediateResults(RewrittenQuery o) {
		...
		for (int i = 0; i < o.getHittingQuerySet().size(); i++) {
			...
			HittingQuerySetGroup.get(HittingQuerySetGroup.size() - 1).add(o.getHittingQuerySet().get(i));
		}
	...
	public String toString() {
		return "IntermediateResults [commonVarMappingList="
				...
				+ HittingQuerySetGroup + ", OriginalVarMapList="
				+ OriginalVarMapList + ", RenamedVarMapList="
				+ RenamedVarMapList + ", ConstraintList=" + ConstraintList + ", resultList.size()="+ resultList.size()+","+Arrays.toString(resultList.get(0))
				+ "]";
	}
	...
	public void distributeResultsInFullQuery(ArrayList<FullQuery> allQueryList) {
		...
		// 击中的查询  循环  Pair [first=0, second=0], Pair [first=0, second=1]   查询，查询里的        三元组
		for (int i = 0; i < this.getHittingQuerySetGroup().size(); i++) {
			int fullQueryIdx = this.getHittingFullQueryIdx(i);  // 击中查询的ID  ，first的值
			FullQuery curFullQuery = allQueryList.get(fullQueryIdx);  // 击中查询的原始查询
//			curFullQuery.clearResults();  // 清空结果列表
			TreeMap<String, Integer> curVarIDMap = curFullQuery.getVarIDMap();  // 原始查询 顶点对应的id
			TreeMap<String, String> tmpRenamedVarMap = this.getRenamedVarMap(i); // 重写查询的重写（顶点）变量对应的真实变量
			int[] mappingPosArr = new int[this.getResultLen()]; // 原始查询中变量的个数
 			String[] mappingFilterArr = new String[this.getResultLen()];
 			Arrays.fill(mappingPosArr, -1);  // 用 -1  填充 mappingPosArr 中的值
			Arrays.fill(mappingFilterArr, ""); //用 “” 填充mappingFilterArr 中的值

			TreeMap<String, String> constraintVarMap = new TreeMap<String, String>();
			for (int j = 0; j < this.ConstraintList.get(i).size(); j++) { // Filter语句 如：[first=?rv_2_0_3, second="Increased ..."]
				constraintVarMap.put(this.ConstraintList.get(i).get(j).first,this.ConstraintList.get(i).get(j).second);
			}
			
			// 原始查询中变量   循环
			for (int j = 0; j < this.getResultLen(); j++) {
				String varStr = "";
 				if(this.Pos2VarMap.get(j).size()-1 >= i){
					 varStr = this.Pos2VarMap.get(j).get(i);
				}else{
					 varStr = this.Pos2VarMap.get(j).get(0);
				} 
//				varStr = this.Pos2VarMap.get(j).get(0); //某个重写查询中的变量
				if (tmpRenamedVarMap.containsKey(varStr)) {
					mappingPosArr[j] = curVarIDMap.get(tmpRenamedVarMap.get(varStr));
				} else if (constraintVarMap.containsKey(varStr)) { // 属于Filter里的变量
					mappingFilterArr[j] = constraintVarMap.get(varStr);  // Filter 里变量对应的值
					if (mappingFilterArr[j].startsWith("<") && mappingFilterArr[j].endsWith(">")) {
						mappingFilterArr[j] = mappingFilterArr[j].substring(1,mappingFilterArr[j].length() - 1); // 去掉 < >
					}
				}
			}
			...
			for (int k = 0; k < this.resultList.size(); k++) {						
				...
				Arrays.fill(tmpRes, "");
				int tag = 0; 
				for (int j = 0; j < this.getResultLen(); j++) {  // 变量的个数
					// 去除查询结果中string的尾缀
					String str = "^^<http://www.w3.org/2001/XMLSchema#string>";
					this.resultList.get(k)[j] = this.resultList.get(k)[j].replace(str, "");
					
					if (mappingPosArr[j] != -1) { // 属于重写后的变量，也就是  顶点变量
						tmpRes[mappingPosArr[j]] = this.resultList.get(k)[j];
						if (tmpRes[mappingPosArr[j]].replace(" ", "").length()==0) {
//							System.out.println("-- 1 --，"+tmpRes[mappingPosArr[j]]);
							tag = 1;
							break;
						} 
					} else if (mappingFilterArr[j].replace(" ", "").length()!=0) { // 属于Filter				
						if (!mappingFilterArr[j].equals(this.resultList.get(k)[j])) {
							tag = 1;
							break;
						}
					}
				...
				if (tag == 0) {
					...
					resultSet.add(tmpResStr);
//					System.out.println(tmpResStr.split(",,,").length);
				}
			}// for  end
			System.out.println("result size : "+resultSet.size());
			
			if(curFullQuery.getResultList().size()>0){
				//假设 查询中有多个三元组之间存在不相关的变量， 存结果的笛卡儿积。
				curFullQuery.addAllMultiResult(resultSet, curVarIDMap.size());
			}else{
	...
	/**
	 * Join 两个查询结果
	 * @param preIntermediateResults
	 * @param curIntermediateResults 是和第一个重写查询Join的查询 
	 * @return
	 */
	public static LinkedList<IntermediateResults> Join(IntermediateResults preIntermediateResults,IntermediateResults curIntermediateResults) {
		...
		while (preIntermediateResults.HittingQuerySetGroup.size() != 0) {
			...
			int tag = 0; 
			for (int thisQueryIdx = 0; thisQueryIdx < preIntermediateResults.HittingQuerySetGroup.size(); thisQueryIdx++) {
				Pair<Integer, Integer> p1 = preIntermediateResults.HittingQuerySetGroup.get(thisQueryIdx).get(0);
				int otherQueryIdx = curIntermediateResults.findSameFullQuery(p1); // cur的第几个pair的查询id和pre的相等

				if (otherQueryIdx != -1) {
					if (newIntermediateResults == null) {
						thisQueryIdxList.add(thisQueryIdx);
						otherQueryIdxList.add(otherQueryIdx);
						// if preIntermediateResults can join with curIntermediateResults by optimized distributed join, then return true
						newIntermediateResults = IntermediateResults.computeMapping(preIntermediateResults,curIntermediateResults, thisQueryIdx,otherQueryIdx);
						// if the common variables are in the main pattern, return true  tag=2; otherwise, return false tag=1
						if (!IntermediateResults.checkCommonVarList(newIntermediateResults)) {
							tag = 1;
							break;
						} else {
							tag = 2;
						...
						if (newIntermediateResults.hasSameJoinScheme(preIntermediateResults, curIntermediateResults,thisQueryIdx, otherQueryIdx)) {
							thisQueryIdxList.add(thisQueryIdx); //?
							otherQueryIdxList.add(otherQueryIdx);
						}
					}
			...
			if (tag != 0) { 
				if (tag == 2) { 
					OPTJoin(preIntermediateResults, curIntermediateResults,newIntermediateResults); 
				} else {
					// reduce the number of results in preIntermediateResults
					ArrayList<String[]> preReducedResList = preIntermediateResults.distributeResultListOfHittingQuery(newIntermediateResults,thisQueryIdxList.get(0));
					ArrayList<String[]> curReducedResList = curIntermediateResults.distributeResultListOfHittingQuery(newIntermediateResults,otherQueryIdxList.get(0));
//					System.out.println(Arrays.toString(preReducedResList.get(0)));
//					System.out.println(Arrays.toString(curReducedResList.get(0)));
					NaiveJoin(preReducedResList, preIntermediateResults,curReducedResList,curIntermediateResults,newIntermediateResults);
				} 
	...
	private ArrayList<String[]> distributeResultListOfHittingQuery(IntermediateResults newIntermediateResults, int idx) {

		ArrayList<String[]> tmpResList = new ArrayList<String[]>();

		// TreeMap<String, Integer> curVarIDMap = this.Var2PosMap;
		TreeMap<String, String> tmpRenamedVarMap = this.RenamedVarMapList.get(idx);
		...
		for (int k = 0; k < this.ConstraintList.get(idx).size(); k++) {
			tmpConstraintMap.put(this.ConstraintList.get(idx).get(k).first,this.ConstraintList.get(idx).get(k).second);
		}
...
//				if (tmpRenamedVarMap.containsKey(varStr)) {  // 也就是重写查询里面的变量。可是有的Filter的变量是不在这个列表里的
				// 也就是Filter 有些变量是重写查询产生的，不是原始查询中的变量改写的
					mappingPosArr[j] = newIntermediateResults.Var2PosMap.get(varStr);
//				}
				if (tmpConstraintMap.containsKey(varStr)) {
					mappingConstraintPosArr[j] = tmpConstraintMap.get(varStr);
					if (mappingConstraintPosArr[j].startsWith("<") && mappingConstraintPosArr[j].endsWith(">")) {
						mappingConstraintPosArr[j] = mappingConstraintPosArr[j].substring(1,mappingConstraintPosArr[j].length() - 1);
					}
				}
			}
		}
		...
		for (int k = 0; k < this.resultList.size(); k++) {
			String[] tmpRes = new String[newIntermediateResults.Pos2VarMap.size()];
			Arrays.fill(tmpRes, "");
			int tag = 0;
			for (int j = 0; j < this.Pos2VarMap.size(); j++) {
	...
	private static void NaiveJoin(ArrayList<String[]> firstReducedResList,
		...
		for (int i = 0; i < firstReducedResList.get(0).length; i++) { // 
			if (!firstReducedResList.get(0)[i].equals("") && !secondReducedResList.get(0)[i].equals("")) {
				first_joining_pos = i;
		...
		while (l_iter < firstReducedResList.size() && r_iter < secondReducedResList.size()) {
//			System.out.println(firstReducedResList.get(l_iter)[first_joining_pos]);
//			System.out.println(secondReducedResList.get(r_iter)[second_joining_pos]);
//			System.out.println(firstReducedResList.get(l_iter)[first_joining_pos].compareTo(secondReducedResList.get(r_iter)[second_joining_pos]));
//			
			if (l_iter == firstReducedResList.size()) { //　？？
				r_iter++;
			} else if (r_iter == secondReducedResList.size()) { 
				l_iter++;
			} else if (firstReducedResList.get(l_iter)[first_joining_pos].compareTo(secondReducedResList.get(r_iter)[second_joining_pos]) < 0) {
				// this.length()-anotherString.length()
				if(l_iter == firstReducedResList.size()-1){
					r_iter++;
				}else{
					l_iter++;
				}
				...
				r_iter++;
					...
					while (l_iter_end < firstReducedResList.size()
							&& firstReducedResList.get(l_iter_end)[first_joining_pos].compareTo(firstReducedResList.get(l_iter)[first_joining_pos]) == 0) {
						l_iter_end++;
					...
					while (r_iter_end < secondReducedResList.size()
							&& secondReducedResList.get(r_iter_end)[second_joining_pos].compareTo(secondReducedResList.get(r_iter)[second_joining_pos]) == 0) {
						r_iter_end++;
					...
					for (int i = l_iter; i < l_iter_end; i++) {
						for (int j = r_iter; j < r_iter_end; j++) {
							...
							for (int k = 0; k < len; k++) {
								...
								if (!firstReducedResList.get(i)[k].equals("")
										&& !secondReducedResList.get(j)[k].equals("")) {
									if (firstReducedResList.get(i)[k].equals(secondReducedResList.get(j)[k])) {
										newRes[k] = firstReducedResList.get(i)[k];
									} else {
										tag = 1;
										break;
									} 
								}else if (!firstReducedResList.get(i)[k].equals("")
										&& secondReducedResList.get(j)[k].equals("")) {
									// 后一个 为空
									newRes[k] = firstReducedResList.get(i)[k];
								}else if (firstReducedResList.get(i)[k].equals("")
										&& !secondReducedResList.get(j)[k].equals("")) {
									// 前一个 为空
									newRes[k] = secondReducedResList.get(j)[k];
								} else {
									newRes[k] = "";
								}
							}
	...
	private static void sortResultInPos(ArrayList<Result> aResultList,int joining_pos) {
		if (joining_pos == 0) {
	...
	private static void OPTJoin(IntermediateResults firstIntermediateResults,
		...
		int first_joining_pos = firstIntermediateResults.findCommonVarPos(newIntermediateResults.commonVarMappingList);
		int second_joining_pos = secondIntermediateResults.findCommonVarPos(newIntermediateResults.commonVarMappingList);
		if(first_joining_pos==-1||second_joining_pos==-1){
			return;
		}
		...
		int[] firstNewIntermediateResults2IntermediateResultsArr = new int[newIntermediateResults.getResultLen()];
		int[] secondNewIntermediateResults2IntermediateResultsArr = new int[newIntermediateResults.getResultLen()];
		for (int i = 0; i < firstNewIntermediateResults2IntermediateResultsArr.length; i++) {
			ArrayList<String> varList = newIntermediateResults.Pos2VarMap.get(i);
			...
			for (int j = 0; j < varList.size(); j++) {
				...
				if (firstIntermediateResults.Var2PosMap.containsKey(varStr)) {
					firstNewIntermediateResults2IntermediateResultsArr[i] = firstIntermediateResults.Var2PosMap.get(varStr);
				}
				if (secondIntermediateResults.Var2PosMap.containsKey(varStr)) {
					secondNewIntermediateResults2IntermediateResultsArr[i] = secondIntermediateResults.Var2PosMap.get(varStr);
				}
			}
		}
		...
		int l_iter = 0, r_iter = 0, len = newIntermediateResults.getResultLen();
		ArrayList<String[]> n_relation = new ArrayList<String[]>();
		while (l_iter < firstIntermediateResults.resultList.size() && r_iter < secondIntermediateResults.resultList.size()) {
//			System.out.println(firstIntermediateResults.resultList.get(l_iter)[first_joining_pos]);
//			System.out.println(secondIntermediateResults.resultList.get(r_iter)[second_joining_pos]);
//			System.out.println(firstIntermediateResults.resultList.get(l_iter)[first_joining_pos].compareTo(secondIntermediateResults.resultList.get(r_iter)[second_joining_pos]));
//			
			if (l_iter == firstIntermediateResults.resultList.size()) {
				...
				while (l_iter_end < firstIntermediateResults.resultList.size()
					...
					l_iter_end++;
					if (l_iter_end == firstIntermediateResults.resultList.size()) {
						break;
					}
				}
	...
	private int findCommonVarPos(ArrayList<ArrayList<String>> curCommonVarMappingList) {
		if(curCommonVarMappingList.size()==0){
			return -1;
		}
	...
	private static IntermediateResults computeMapping(
		...
		newIntermediateResults.copyVarPosMap(thisIntermediateResults.Pos2VarMap);

		Iterator<String> iter = secondRenamedVarSet.iterator();
		while (iter.hasNext()) {
			String renamedVarStr = iter.next();
			String originalVarStr = otherIntermediateResults.RenamedVarMapList.get(otherQueryIdx).get(renamedVarStr);
			if (tmpCommonVarList.contains(originalVarStr)) {
				ArrayList<String> firstRenamedVarList = thisIntermediateResults.OriginalVarMapList
						.get(thisQueryIdx).get(originalVarStr);
				for (int renamedVarIdx = 0; renamedVarIdx < firstRenamedVarList.size(); renamedVarIdx++) {
					newIntermediateResults.Var2PosMap
							.put(renamedVarStr,newIntermediateResults.Var2PosMap.get(firstRenamedVarList.get(renamedVarIdx)));
					if (!newIntermediateResults.Pos2VarMap.get(newIntermediateResults.Var2PosMap.get(firstRenamedVarList.get(renamedVarIdx))).contains(renamedVarStr)) {

						newIntermediateResults.Pos2VarMap.get(newIntermediateResults.Var2PosMap.get(firstRenamedVarList.get(renamedVarIdx)))
						.add(renamedVarStr);
					}
				}
			} else {
	...
	private int findSameFullQuery(Pair<Integer, Integer> p1) {
		for (int i = 0; i < this.HittingQuerySetGroup.size(); i++) {
			if (this.HittingQuerySetGroup.get(i).get(0).first.equals(p1.first)) {
				return i;
			}
		}
		return -1;
	...
	/**
	 * 判断是否可以join
	 * @param curIntermediateResults
	 * @param curRewrittenQuery   重写列表的一个重写查询
	 * @return
	 */
	public static boolean canJoin(IntermediateResults curIntermediateResults,RewrittenQuery curRewrittenQuery) {
		// 当前重写的查询击中的一些查询
		for (int thisQueryIdx = 0; thisQueryIdx < curRewrittenQuery.getHittingQuerySet().size(); thisQueryIdx++) {
			// 当前击中的查询
			Pair<Integer, Integer> p1 = curRewrittenQuery.getHittingQuerySet().get(thisQueryIdx);
			// 当前重写的查询和击中的查询是否相同
			int otherQueryIdx = curIntermediateResults.findSameFullQuery(p1);
			// -1  没有相同的查询，
			if (otherQueryIdx != -1) {
				Set<String> firstOriginalVarSet = curRewrittenQuery.getOriginalVarMapList().get(thisQueryIdx).keySet();
				Set<String> secondOriginalVarSet = curIntermediateResults.OriginalVarMapList.get(otherQueryIdx).keySet();
