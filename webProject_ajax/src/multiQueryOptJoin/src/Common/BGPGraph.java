...
public class BGPGraph {
	...
	public void setAdjacencyMatrix(TreeMap<Integer, TreeMap<Integer, String>> adjacencyMatrix) {
...
//			System.out.println("【why】"+o.VertexIDmap.size()+" : "+this.VertexIDmap.size());
			return null;
		}
					...
					for (int j = 0; j < cur_pos; j++) {
						...
						if (o.AdjacencyMatrix.containsKey(j)
								&& o.AdjacencyMatrix.get(j)
										.containsKey(cur_pos)) {
							if (this.AdjacencyMatrix.containsKey(newState[j]) && this.AdjacencyMatrix.get(newState[j]).containsKey(newState[cur_pos])) {
								if (!o.AdjacencyMatrix.get(j).get(cur_pos).equals(this.AdjacencyMatrix.get(newState[j]).get(newState[cur_pos]))) {
									tag = 1;
									break;
								}
							} else {
	...
	/**
	 * 返回主模式的常量在这个查询的对应的ID，默认[0,-1],表示没有
	 * @param sub
	 * @return newState
	 */
					...
					for (int j = 0; j < cur_pos; j++) {
						...
						if (sub.AdjacencyMatrix.containsKey(j)
							...
							if (this.AdjacencyMatrix.containsKey(newState[j])
									&& this.AdjacencyMatrix.get(newState[j]).containsKey(newState[cur_pos])) {
...
/*		System.out.println(curState.length+" curState: ");
		for(Integer i=0;i<curState.length;i++){
			System.out.print(curState[i]+" ");
		}
		System.out.println();
		System.out.println(newState.length+" newState: ");
		for(Integer i=0;i<newState.length;i++){
