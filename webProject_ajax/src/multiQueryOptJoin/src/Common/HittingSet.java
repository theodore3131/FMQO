...
public class HittingSet implements Comparable<HittingSet> {
	private ArrayList<TriplePattern> triplePatternList;  // 三元组
	private ArrayList<Pair<Integer, Integer>> HittingQuerySet;  // 击中的 查询
	...
	public String toString() {
		return "HittingSet [triplePatternList=" + triplePatternList+ ", HittingQuerySet=" + HittingQuerySet + "]";
	}
	...
	public void initializeTriplePattern(TriplePattern tp) {
		...
		if (!this.adjacentList.containsKey(newTp.getSubjectStr())) {
			this.adjacentList.put(newTp.getSubjectStr(),new ArrayList<EdgeInfo>());
		}
		if (!this.adjacentList.containsKey(newTp.getObjectStr())) {
			this.adjacentList.put(newTp.getObjectStr(),new ArrayList<EdgeInfo>());
		}

		this.adjacentList.get(newTp.getSubjectStr()).add(new EdgeInfo(newTp.getPredicateStr(), true, newTp.getObjectStr()));
		this.adjacentList.get(newTp.getObjectStr()).add(new EdgeInfo(newTp.getPredicateStr(), false, newTp.getSubjectStr()));
	}
