...
public class FullQuery {  
	...
	public void addAllMultiResult(TreeSet<String> resultSet,int varSize) {
		ArrayList<String[]> joinResult = new ArrayList<String[]>();
		for (int k = 0; k < this.getResultList().size(); k++) {						
			String[] tmpRes = new String[varSize];
			Arrays.fill(tmpRes, "Null");
			Iterator<String> iter = resultSet.iterator();
			String[] curResult = this.getResultList().get(k);
			for(int j=0;j<varSize;j++){
				if(!curResult[j].equals("Null") && tmpRes[j].equals("Null")){
					tmpRes[j] = curResult[j];
				}
			}
			while (iter.hasNext()) {
				String[] str = iter.next().split(",,,");
				for(int j=0;j<varSize;j++){
					if(!str[j].equals("Null") && tmpRes[j].equals("Null")){
						tmpRes[j] = str[j];
					}
				}
				joinResult.add(tmpRes);
			}			
		}
		this.resultList = joinResult;
	}
	...
	public void reduceSourcesQTree(
		...
		int[][] JGAdjacentMatrix = new int[this.localQueryList.size()][this.localQueryList.size()];
		ArrayList<ArrayList<Integer>> newSourceList = new ArrayList<ArrayList<Integer>>();
